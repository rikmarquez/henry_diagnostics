...
const opportunitySchema = z.object({
  vehicle_id: z.number().int().positive('ID de vehículo inválido'),
  ...
  kilometraje_referencia: z.number().int().min(0, 'Kilometraje no puede ser negativo').optional(),
});

const updateOpportunitySchema = opportunitySchema.partial().omit({ vehicle_id: true });
...
const appointmentSchema = z.object({
  ...
  cita_descripcion_breve: z.string().min(1, 'Descripción del vehículo requerida'),
  cita_telefono_contacto: z.string().min(1, 'Teléfono de contacto requerido'),
  cita_nombre_contacto: z.string().min(1, 'Nombre de contacto requerido'),
  titulo: z.string().optional(),
  descripcion: z.string().optional(),
});

const noteSchema = z.object({
  tipo_contacto: z.enum(['llamada', 'whatsapp', 'visita', 'email', 'nota_interna']).optional(),
  resultado: z.enum(['contactado', 'no_contesta', 'ocupado', 'interesado', 'no_interesado', 'agendado']).optional(),
...
export const createOpportunity = async (req: AuthRequest, res: Response) => {
  try {
    ...
    const vehicleExists = await query('SELECT vehicle_id FROM vehicles WHERE vehicle_id = $1 AND activo = true', [opportunityData.vehicle_id]);
    if (vehicleExists.rows.length === 0) {
      return res.status(404).json({ message: 'Vehículo no encontrado' });
    }

    // Verificar que el cliente existe
    const customerExists = await query('SELECT customer_id FROM customers WHERE customer_id = $1', [opportunityData.customer_id]);
    ...
    if (opportunityData.usuario_asignado) {
      const userExists = await query('SELECT user_id FROM users WHERE user_id = $1 AND activo = true', [opportunityData.usuario_asignado]);
...
export const searchOpportunities = async (req: AuthRequest, res: Response) => {
  try {
    ...
    if (vehicle_id) {
      whereConditions.push(`o.vehicle_id = $${paramIndex}`);
...
export const updateOpportunity = async (req: AuthRequest, res: Response) => {
  try {
    ...
    const opportunityExists = await query('SELECT opportunity_id FROM opportunities WHERE opportunity_id = $1', [opportunityId]);
    ...
    if (updateData.usuario_asignado) {
      const userExists = await query('SELECT user_id FROM users WHERE user_id = $1 AND activo = true', [updateData.usuario_asignado]);
      if (userExists.rows.length === 0) {
        return res.status(404).json({ message: 'Usuario asignado no encontrado' });
      }
    }
    ...
    Object.entries(updateData).forEach(([key, value]) => {
      if (value !== undefined) {
        if (key === 'fecha_sugerida' && value) {
          ...
          const fechaSugerida = new Date(value as string);
...
export const addOpportunityNote = async (req: AuthRequest, res: Response) => {
  try {
    const opportunityId = parseInt(req.params.id);
    if (isNaN(opportunityId)) {
      return res.status(400).json({ message: 'ID de oportunidad inválido' });
...
export const getOpportunitiesByVehicle = async (req: AuthRequest, res: Response) => {
  try {
    ...
    console.error('Error obteniendo oportunidades por vehículo:', error);
    res.status(500).json({ message: 'Error interno del servidor' });
  }
};

export const getRemindersToday = async (req: AuthRequest, res: Response) => {
  try {
...
export const createAppointment = async (req: AuthRequest, res: Response) => {
  try {
    ...
    // FLUJO CORRECTO: Crear cita rápida SIN vehículo ni cliente
    // Los campos vehicle_id y customer_id quedan NULL hasta que llegue el cliente
    const result = await query(`
      INSERT INTO opportunities (
      ...
      RETURNING *
    `, [
      null, // vehicle_id = NULL para citas rápidas
      null, // customer_id = NULL para citas rápidas  
      req.user?.user_id,
      'cita_agendada',
...
export const convertOpportunityToAppointment = async (req: AuthRequest, res: Response) => {
  try {
    const opportunityId = parseInt(req.params.id);
    if (isNaN(opportunityId)) {
      return res.status(400).json({ message: 'ID de oportunidad inválido' });
