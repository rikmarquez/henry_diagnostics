...
import { 
  ...
  ConvertOpportunityToCitaRequest,
  Vehicle,
  Customer,
  Service,
  Opportunity
 ...
 */
export const processWalkInClient = async (req: AuthRequest, res: Response) => {
  ...
  try {
    ...
    if (data.vehiculo_existente_id) {
      ...
      const placaExistente = await client.query(
        'SELECT vehicle_id FROM vehicles WHERE placa_actual = $1 AND activo = true',
 ...
 */
export const convertOpportunityToCita = async (req: AuthRequest, res: Response) => {
  try {
    const data: ConvertOpportunityToCitaRequest = req.body;
    ...
    const customerResult = await query(
      'SELECT nombre, telefono FROM customers WHERE customer_id = $1',
 ...
 */
export const receptionarCita = async (req: AuthRequest, res: Response) => {
  const client = await getClient();
 ...
 */
export const getCitasParaRecepcion = async (req: AuthRequest, res: Response) => {
  try {
